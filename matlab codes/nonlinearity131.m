...
trunc  = 150; % the length of the duration of listening.
% neurons_num = 15 ; % for LSTM 1-1-1
 neurons_num = 28 ; % for LSTM 1-2-1
%  neurons_num = 41 ; % for LSTM 1-3-1
...
if Show
    figure(1)
    plot(1:trunc,target_trunc(end-trunc+1:end),'.r',1:trunc,matrix(gateId,end-trunc+1:end),'v');
    ax1 = gca;
    ...
    RMSE = sqrt(mean((target_trunc(end-trunc+1:end)-matrix(gateId,end-trunc+1:end)).^2,2 ));
    sc = sprintf('weight 2 \n error : %f', RMSE);
...
data = matrix(gateId,end-trunc+1:end); %gate values 22 23 for LSTM 1-2-1 
...
data3 =  reshape(data2, 7,length(data2)/7); % each column represents a set. 
...
positions2 =  reshape(positions, 7,length(positions)/7); % each column represents a triple. 
...
ax1 =gca;
%create xticklabels
labels= num2str(round(posAscend(:,1)*10)/10); 
...
ax2 =gca;
%create xticklabels
labels= num2str(round(posDescend(:,1)*10)/10); 
