...
import EventSegmentationArchitecture.ForwardModel;
import EventSegmentationArchitecture.ModelFitter;
import EventSegmentationArchitecture.ModelList;
import EventSegmentationScenario.MyDemoLauncher.MyDemo;
import EventSegmentationScenario.Arm;
import EventSegmentationScenario.MyDemoLauncher;
...
public class Arm implements MyDemo {
	...
	double[][] testingPos = { { 37.5, 37.5 }, { 37.5, 67.5 }, { 67.5, 37.5 },
			{ 67.5, 67.5 } };
	...
	ExampandableIntMatrix predictedModelsHeavyBurrdock = new ExampandableIntMatrix(
			8);
	...
	DiskType upperJointType = new DiskType(
			DiskMaterial.DOUGH.withColor(Color.RED));
	DiskType lowerJointType = new DiskType(
			DiskMaterial.DOUGH.withColor(Color.MAGENTA));
	DiskType limbType = new DiskType(DiskMaterial.DOUGH.withColor(Color.BLUE));
	DiskType handJointType = new DiskType(
			DiskMaterial.DOUGH.withColor(Color.GREEN));
	...
	double[] lastHandPos = new double[2]; // to switch commands when hand stays
											// still
	...
	public Environment getEnvironment() {
		...
		upperArmJoints[0].setRange(JointActionType.ActionType.SPIN,
				Math.toRadians(-80), 0);
		d = upperArmJoints[0];
		allDisks.add(d);
		...
		for (int i = 0; i < numUpperArmLimbs; i++) {
			if (i > 0) {
				upperArmJoints[i] = d.attachJoint(0, jointSize, 0,
						upperJointType);
				// limit all joints
				if (i % 2 == 0) {
					upperArmJoints[i].setRange(JointActionType.ActionType.SPIN,
							Math.toRadians(-160), Math.toRadians(0));
				} else {
					upperArmJoints[i].setRange(JointActionType.ActionType.SPIN,
							Math.toRadians(0), Math.toRadians(160));
				}
				d = upperArmJoints[i];
				allDisks.add(d);
			}
		...
		upperArmJoints[numUpperArmLimbs] = d.attachJoint(0, jointSize, 0,
				upperJointType);
		if (numUpperArmLimbs % 2 == 0) {
			upperArmJoints[numUpperArmLimbs].setRange(
					JointActionType.ActionType.SPIN, Math.toRadians(-80),
					Math.toRadians(0));
		} else {
			upperArmJoints[numUpperArmLimbs].setRange(
					JointActionType.ActionType.SPIN, Math.toRadians(0),
					Math.toRadians(80));
		}
		...
		forearmJoints[0] = d.attachJoint(0, jointSize, Math.toRadians(90),
				lowerJointType);
		forearmJoints[0].setRange(JointActionType.ActionType.SPIN,
				Math.toRadians(10), Math.toRadians(90));
		d = forearmJoints[0];
		allDisks.add(d);

		for (int i = 0; i < numForearmLimbs; i++) {
			if (i > 0) {
				forearmJoints[i] = d.attachJoint(0, jointSize, 0,
						lowerJointType);
				// limit all joints
				if (i % 2 == 0) {
					forearmJoints[i].setRange(JointActionType.ActionType.SPIN,
							Math.toRadians(-160), Math.toRadians(0));
				} else {
					forearmJoints[i].setRange(JointActionType.ActionType.SPIN,
							Math.toRadians(0), Math.toRadians(160));
		...
		forearmJoints[numForearmLimbs] = d.attachJoint(0, jointSize, 0,
				lowerJointType);
		if (numForearmLimbs % 2 == 0) {
			forearmJoints[numForearmLimbs].setRange(
					JointActionType.ActionType.SPIN, Math.toRadians(-80),
					Math.toRadians(0));
		} else {
			forearmJoints[numForearmLimbs].setRange(
		...
		// create the hand
		handjoint = d.attachJoint(0, jointSize * 3.5, Math.PI / 2,
				handJointType);
		handjoint.setRange(JointActionType.ActionType.SPIN,
				Math.toRadians(-90), Math.toRadians(90));
		allDisks.add(handjoint);
		handjoint.setZLevel(1);
		/*
		 * Disk firstHand = handjoint.attachDisk(0, handSize, handType); d =
		 * firstHand; allDisks.add(d); int handLength = 12; for(int i = 0; i <
		 * handLength; i++){ d = d.attachDisk(0, handSize, handType);
		 * d.setZLevel(1); allDisks.add(d); } d =
		 * firstHand.attachDisk(Math.PI/2, handSize, handType); for(int i = 1; i
		 * < handLength; i++){ d = d.attachDisk(0, handSize, handType);
		 * allDisks.add(d); d.setZLevel(1); }
		 */

		// determine min and max Handposition
		minPosX = body.getX() + bodySize + numUpperArmLimbs * jointSize
				+ minDistanceToObjects;
		minPosY = body.getY() + bodySize + numForearmLimbs * jointSize
				+ minDistanceToObjects;
		maxPosX = handjoint.getX() - minDistanceToObjects;
		maxPosY = handjoint.getY() - minDistanceToObjects;

		this.setFloor(env, FloorCellType.FULL, FloorCellType.FULL,
				FloorCellType.BURRKILLER);

		this.object = new HeavyBurrdock(env, 45, 93, 0, handJointType, this);// Burrdock(env,
																				// 35,
																				// 56,
																				// handType,
																				// this);
		models = new ModelFitter(xSize, getSensation(), modelType,rs);
		if (readModelList ){
			String BinPath = "/Users/pohsuanhuang/Desktop/Curriculum Section/Butz Lab/Java Code/DiskWorldEAR/ModelList.bin";
			File log = new File(BinPath);
			if (log.exists() ) {
			models.readWeights();
			}
		}
	...
	public AgentMapping[] getAgentMappings() {
		...
		actions.add(upperArmJoints[0].createJointAction("upperarmJoint#0",
				maxAngularChange, JointActionType.ActionType.SPIN,
				JointActionType.ControlType.CHANGE)); // TARGET
		// other joints are controlled by angle change
		for (int i = 1; i < upperArmJoints.length; i++) {
			actions.add(upperArmJoints[i].createJointAction("upperarmJoint#"
					+ i, maxAngularChange, JointActionType.ActionType.SPIN,
					JointActionType.ControlType.CHANGE));
		}
		for (int i = 0; i < forearmJoints.length; i++) {
			actions.add(forearmJoints[i].createJointAction("forearmJoint#" + i,
					maxAngularChange, JointActionType.ActionType.SPIN,
					JointActionType.ControlType.CHANGE));
		}
		actions.add(handjoint.createJointAction("hand", maxAngularChange,
				JointActionType.ActionType.SPIN,
				JointActionType.ControlType.CHANGE));
	...
	public boolean objectToBeRemoved() {
		...
		if (object.getType() == FLY) {
			if (object.getX() > 96 || object.getY() > 96 || object.getX() < 4
					|| object.getY() < 4) {
				return true;
			}
		}
		...
		return euclidianDistance(object.getX(), object.getY(), 52.5, 52.5) < 2.0;// (object.getY()
																					// >=
																					// burrdockKillerMinY
																					// &&
																					// object.getY()
	...
	public double[] generateNewObjectPosition(int objectType) {
		if (!training) {
			System.out
					.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
			System.out.println("ObjTested " + objectsTested
					+ ", In TestingSet "
					+ ((int) testingSet.get(objectsTested)) + ", pos "
					+ ((int) testingSet.get(objectsTested)) % 100);
			return testingPos[((int) testingSet.get(objectsTested)) % 100];
		}
		/*
		 * if(objectType == FLY){ return generateNewObjectPositionFly(); }
		 */// TODO
	...
	private double[] generateNewObjectPositionBurrdock() {
		...
		while (notFinished) {
			...
			pos[0] = rand.nextDouble() * (this.maxPosX - this.minPosX)
					+ minPosX;
			pos[1] = rand.nextDouble() * (this.maxPosY - this.minPosY)
					+ minPosY;
			notFinished = false;
			for (Disk d : allDisks) {
				if (euclidianDistance(pos[0], pos[1], d.getX(), d.getY()) < objectRadius
						+ d.getRadius()) {
					notFinished = true;
				}
			}

			if (pos[0] <= burrdockKillerMaxX + distanceToPit
					&& pos[0] >= burrdockKillerMinX - distanceToPit
					&& pos[1] >= burrdockKillerMinY - distanceToPit
					&& pos[1] <= burrdockKillerMaxY + distanceToPit) {
				notFinished = true;
			}
	...
	private double[] generateNewObjectPositionFly() {
		...
		while (notFinished) {
			...
			pos[0] = rand.nextDouble() * (this.flyAreaMaxX - this.flyAreaMinX)
					+ flyAreaMinX;// - (this.mouth.getX() +
									// this.mouth.getRadius())) +
									// (this.mouth.getX() +
									// this.mouth.getRadius());
			pos[1] = rand.nextDouble() * (this.flyAreaMaxY - this.flyAreaMinY)
					+ flyAreaMinY;// * ((this.sting.getY() - 2*
									// this.sting.getRadius()) -
									// (this.mouth.getY() +
									// this.mouth.getRadius())) +
									// (this.mouth.getY() +
									// this.mouth.getRadius());
	...
	public double[] pseudoRandomMovement(int time) {
		...
		if (Math.abs(lastHandPos[0] - handjoint.getX()) < 0.00001
				|| /* && */Math.abs(lastHandPos[1] - handjoint.getY()) < 0.00001) {
			timeMovePerformed = 0;
		}
	...
	public MyAgentController[] getControllers() {
		...
		MyAgentController controller = new MyAgentController() {
			...
			public void doTimeStep(double[] sensorValues,
				...
				if (objectToBeRemoved()
					...
					if (initialPhase) {
						...
						if (training) {
							...
							if (object.getType() == FLY) {
								...
								timesForHeavyBurrdock
										.add(500 - timeForInteraction);
							}
						}
					}
					...
					if (stretching) {
						...
						int testingStimuli = (int) testingSet
								.get(objectsTested);
					...
					if (newType == HEAVYBURRDOCK) {
						object = new HeavyBurrdock(env, pos[0], pos[1], 0,
								handJointType, arm);
						newAction = GRABHEAVYBURRDOCK;
					} else if (newType == BURRDOCK) {
						object = new Burrdock(env, pos[0], pos[1], 0,
								handJointType, arm);
				...
				/**
				 * print the mean prediction error to evaluate the performance.
				 * if(!training){ final double meanError =
				 * models.getMeanError(); printMeanError(meanError, stretching);
				 * 
				 * } ------
				 */
				if (stretching == true && stretchTime <= 0
						&& timeForInteractionStretch) {
					stretching = false;
					timeForInteractionStretch = false;
					timeForInteraction = 0;
				}

				System.out.println("Training " + training + ", Stretching "
						+ stretching + ", Stretch Time " + stretchTime
				...
				if (stretching || alwaysSupervised) {
					...
					if (timeForInteraction > 0 || !training)
						forwardX = models.goalDirectedInverseKinematics(
								getSensation(), 2);
					else
						forwardX = supervisedControll();
				}
				/*
				 * else{ Double[] goalSensation =
				 * scenario.getDesiredSensation(time, object); Double[]
				 * deltaGoal = getDesiredSensationalChange(getSensation(),
				 * goalSensation); forwardX = models.forwardModel(deltaGoal); }
				 */
				...
				if (slowDown > 0) {
					...
					slowDown--;
						...
						&& body.getDiskComplex().equals(
								object.getDisk().getDiskComplex())) {
					for (int i = 0; i < commands.length; i++)
				...
				for (int i = upperArmJoints.length; i < upperArmJoints.length
						+ forearmJoints.length; i++) {
					if (i == upperArmJoints.length
							|| i == upperArmJoints.length + numForearmLimbs) {
			...
			public void doAfterTimeStep() {
				...
				// Dont use modelRecommendations if you want the model to learn
				// by itself
				// use only for debugging purposes or if you want to compare the
				// exact models used
				if (true) {
					if (object.getType() == FLY) {
						if (object.hasMoved()) {
				...
				// models.doTimeStepModelForcing(x, y, newSensation, time,
				// modelRecommendations, objectSwitched);
				...
				models.doTimeStep(x, y, newSensation,(training && !stretching), modelRecommendations,	modelType);
	...
	private double[] getSensationMultpileObjects() {
		...
		// For Fly:
		sensation[6] = euclidianDistance(object.getX(), object.getY(), 52.5,
				52.5); // TODO
	...
	public Double[] getDesiredSensationalChange(double[] currentSensation,
			Double[] goalSensation) {
	...
	public void setTimeForInteraction() {
		/*
		 * this.printAfterAction(timeActionPerformed); timeForInteraction = 350;
		 * timeActionPerformed = 0; if(object.getType() == FLY){ currentAction =
		 * PUSHFLY; } else if (object.getType() == BURRDOCK){ currentAction =
		 * CARRYBURRDOCK; } else{ currentAction = CARRYHEAVYBURRDOCK; }
		 */
	}
	 ...
	 * 
	 * @see
	 * Hiwi.MyDemoLauncher.MyDemo#adaptVisualisationSettings(diskworld.visualization
	...
	public void setFloor(Environment env, FloorCellType type1,
			FloorCellType type2, FloorCellType burrKiller) {
		for (int i = 0; i < env.getFloor().getNumX(); i++) {
			for (int j = 0; j < env.getFloor().getNumY(); j++) {
				if (i >= burrdockKillerMinX && i <= burrdockKillerMaxX
						&& j >= burrdockKillerMinY && j <= burrdockKillerMaxY) {
					env.getFloor().setType(i, j, burrKiller);
				} else {
					env.getFloor().setType(
							i,
							j,
							(i >= minPosX && i <= maxPosX)
									&& (j >= minPosY && j <= maxPosY) ? type1
									: type2);
				}
	...
	public void printAfterAction() throws ClassNotFoundException, IOException {
		...
		for (int i = 0; i < timesForBurrdock.size(); i++) {
			...
			// ((double)errorsForBurrdock.get(i));
		...
		for (int i = 0; i < timesForHeavyBurrdock.size(); i++) {
			ssHeavyBurrdock[0] = ssHeavyBurrdock[0]
					+ ((int) timesForHeavyBurrdock.get(i));
			// seHeavyBurrdock[0] = seHeavyBurrdock[0] +
			// ((double)errorsForHeavyBurrdock.get(i));
	...
	/*
	 * public void predictModels(){ if(training || stretching){ return; }
	 * ExampandableIntMatrix m = this.predictedModelsFly; if(object.getType() ==
	 * BURRDOCK){ m = this.predictedModelsBurrdock; } else if (object.getType()
	 * == HEAVYBURRDOCK){ m = this.predictedModelsHeavyBurrdock; }
	 * 
	 * for(int i = 0; i < ySize; i++){ int model =
	 * models.predictNextModel(getSensation(), i); m.add(model, i); } }
	 */
	...
	public void printErrors(Double errors, int[] models) throws ClassNotFoundException, IOException {
		...
		// }
		if (errors != null){
		String[] ss = { errors.toString()  };
		//System.out.println("shorts" + ss[0]);
		Fileprinter.printInFile("SumofErrors" , ss, rs);
		}
