...
public class LinearRLSModel implements ForwardModel {
	...
	public String getName(){
		String name = "LinearRLSModel";
		return name;
	}
	
	/**convert the 2d weight matrix into 1d weight matrix*/
	public double[] getWeights() {
		double[] wei = new double[ weights.length* weights[0].length];
		for (  int row=0; row< weights.length; row++){
			for (int col= 0; col <weights[0].length; col++ ){
			wei[ weights.length* row +col] = weights[row][col];
			}
		}
		return wei;
	}
