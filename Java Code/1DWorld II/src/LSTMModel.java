...
public class LSTMModel implements ForwardModel {
	...
	public void update(double[] motorCommand, double[] sensation) {
		...
		/** 
		 Pass down the content of buffers. The oldest one is omitted from the buffers.  
		 */
		for (int t = 0; t < T; t++) {
			...
			if (t == 0) {
				...
				for (int i = 0; i < dimOut; i++) {temporaryTarget.set(t, i, sensation[i]);
				}
			} else if ( t != T-1 ) {
				// shift the time window by one.
				net.rebuffer(t);
				net.getOutputBuffer(t-1);
				BadHack.getInputBuffer(net, t-1);
			} else {	}
		...
		for (int t = 0; t < T; t++) {
			...
			if (t == 0) {
				...
				for (int i = 0; i < dimIn; i++) {
					temporaryInput.set(t, i, motorCommand[i]);	
					temporaryInput.set(t+1, i, input.get(t,i));
					}
				for (int i = 0; i < dimOut; i++) {
					temporaryTarget.set(t, i, sensation[i]);
