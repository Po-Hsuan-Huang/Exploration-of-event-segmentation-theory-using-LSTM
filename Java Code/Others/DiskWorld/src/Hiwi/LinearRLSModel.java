 ...
 */
public class LinearRLSModel {
	//Alles private, alles final
	...
	public int errorsSize = 100; //1000
	...
	public LinearRLSModel(final int dimIn,
			final double[] initialPrediction, final boolean intercept,
			final double initialGain, final double lambda) {
		this(dimIn, initialPrediction.length, intercept, initialGain, lambda);
		if (intercept) {
			// initialize weights
	...
	public void updateError(double error){
		...
		/*double oldMeanError = this.meanError;
		this.meanError = (this.meanError * this.errorUpdates + error)/((double)(this.errorUpdates + 1));
		if(errorUpdates > 1){
			this.errorVariance = (((double)errorUpdates - 1)/((double)errorUpdates)) *this.errorVariance 
					+ ((error - meanError) * (error - meanError))/((double) errorUpdates + 1);
			
		}
		this.errorUpdates++;*/
		
		/*this.errorUpdates++;
		double delta = error - this.meanError;
		this.meanError += delta/((double) errorUpdates);
		if(errorUpdates > 1)
			this.errorVariance += delta * (error - meanError);*/
	...
	public LinearRLSModel copy(){
		LinearRLSModel clone = new LinearRLSModel(dimIn, prediction, intercept, initialGain, lambda);
		...
		//clone.gainMatrix = this.gainMatrix.clone();
	...
	public void killErrors(){
		this.errors = new LinkedList<Double>();
	}
	
	public static double euclidianDistance(double[] x, double[] y, int from, int to){
		double res = 0;
	...
	public static void main(String[] args){
		double[] initialPred= {0.0, 0.0, 0.0, 0.0};
		LinearRLSModel rls = new LinearRLSModel(2, initialPred , false, 1000, 1.0);
		double oldSensX = 0;
		double oldSensY = 0;
		double rise = 1.0;
		for(int i = 0; i < 10000; i++){
			double[] x = {Math.random(), Math.random()};
			double[] p = rls.predict(x);
			...
			oldSensY = y[1];
			rls.update(x, y);
			System.out.println(euclidianDistance(p, y, 0, 2));
		}
	}
}