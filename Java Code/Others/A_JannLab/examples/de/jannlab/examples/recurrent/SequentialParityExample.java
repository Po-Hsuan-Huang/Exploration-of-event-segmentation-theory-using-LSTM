...
/**
 * The sequential parity problem is a typical sequence classification problem,
 * often used to demonstrate the ability of recurrent networks to "connect"
 * input events over time. Given a sequence over {0, 1} the task is to determine
 * either the number of 1s is even or odd. In this example the problem is
 * learned by an RNN via RandomSearch. <br>
 * </br>
 * 
...
public class SequentialParityExample {
	...
	public static double[] generateSequence(final int length, final Random rnd) {
	...
	public static Sample generateSample(final int length, final Random rnd) {
		//
	...
	private static SampleSet generateSamples(final int samples,
			final int length, final Random rnd) {
	...
	public static double runExperiment(final SampleSet trainset,
			final SampleSet testset, final Net net, final Random rnd,
			final int epochs, final double lbd, final double ubd) {
		//
		// setup network.
		//
		final int maxlength = Math.max(trainset.maxSequenceLength(),
				testset.maxSequenceLength());
	...
	public static void main(String[] args) {
		...
		// build sample sets.
		//
		final SampleSet trainset = generateSamples(trainset_size, length, rnd);
		System.out.println(trainset);
		final SampleSet testset = trainset;
		//
		// evaluate learning success.
		//
		final double rnnerror = runExperiment(trainset, trainset, rnn, rnd,
				epochs, weightslbd, weightsubd);
