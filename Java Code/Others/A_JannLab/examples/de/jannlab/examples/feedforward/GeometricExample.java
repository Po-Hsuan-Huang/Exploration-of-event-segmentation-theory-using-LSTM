...
/**
 * This examples demonstrates the generalization ability of neural networks on
 * learning geometric figures (here classification of circles). Through a
 * visualization during the training, the learning process (convergence) can be
 * observed. The visualization also illustrates out well the effect of an
 * insufficient number of hidden neurons . <br>
 * </br> The samples are points positioned on the uni-square and each point
 ...
 * separable. The more colored the background at a position (x,y) is, the more
 * the MLP "thinks" this point would correspond to the first class.
 * 
...
public class GeometricExample {
	...
	public static SampleSet generateData(final int samples, final float rel,
			final Random rnd) {
		//
		...
		while (size > 0) {
			...
			if (l1.length() > r1 && l2.length() > r2 && l3.length() > r3) {
				...
				//
				if (size2 > 0) {
					result.add(new Sample(new double[] { v.x, v.y },
							new double[] { 0.0 }));
					size2--;
				}
			} else {
	...
	public static void main(String[] args) {
		...
		//
		final SampleSet trainset = generateData(trainset_size + testset_size,
				0.5f, rnd);
		...
		trainer.addListener(new NetTrainerListener() {
			...
			public void epoch(NetTrainer trainer) {
				...
				//
				if ((ep) % (epochs / 16) != 0 && ep != 1)
					return;
				final BufferedImage img = new BufferedImage(800, 800,
				...
				for (Sample s : trainset) {
					int x = (int) ((img.getWidth() - 1) * s.getInput()[0]);
					int y = (int) ((img.getHeight() - 1) * s
							.getInput()[1]);
				...
				JPanel panel = new JPanel() {
					...
					protected void paintComponent(Graphics gfx) {
						...
						gfx.drawImage(img, 0, 0, img.getWidth(),
								img.getHeight(), null);
					}
				};
		...
		System.out.println("recognition accuracy on trainset: "
				+ (val.ratio() * 100.0) + "%");
		//
		val.reset();
		//
		...
		System.out.println("recognition accuracy on testset: "
				+ (val.ratio() * 100.0) + "%");
