...
/**
 * This examples shows how to learn the parity problem with multilayer
 * perceptrons (MLPs) using gradient descent with momentum term. This problem
 * differs from the sequential parity problem, a well known toy problem for
 * recurrent networks, in such a way, that samples are not sequences with
 * variable lengths, but simple equal-sized patterns, which can be learn by
 * MLPs. Therefore we refer here as static parity problem. <br>
 * </br><br>
 * </br> For a given number of bits n, 2^n samples containing every possible bit
 ...
 * samples are used as train set. <br>
 * </br><br>
 * </br> This example also provides to observe the effect of over-fitting when
 * using too large hidden layers. <br>
 * </br>
 * 
